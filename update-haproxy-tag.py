...
import subprocess
import logging
from haproxy_autoscale import get_running_instances_by_tag, file_contents, generate_haproxy_config, steal_elastic_ip
import urllib2


def main():
    # Parse up the command line arguments.
    parser = argparse.ArgumentParser(description='Update haproxy to use all instances running with an aws:autoscaling:groupName tag of the given value.')
    parser.add_argument('--tag', required=True, nargs='+', type=str)
